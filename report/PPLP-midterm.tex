...
Similar to above, two parties can find out the combined cardinalities for common nodes between the parties. Assume again, Alice (graph $G_A$) and Bob (graph $G_B$), can first perform PSI on the nodes of the graph to obtain nodes which they could find their cardinalities. Below the steps to determine the combined cardinality of node $A$:
\begin{itemize}
